...
void Graphics::DrawLines(std::vector<sf::Vector2f> points, sf::Color color)
{
	assert(points.size() > 1);
	sf::VertexArray lines(sf::Lines, points.size());
...
sf::Vector2f Graphics::AddDefaultTranslation(sf::Vector2f vector) const
{
...
sf::Vector2f Graphics::AddDefaultRotation(sf::Vector2f vector) const
{
...
sf::Vector2f Graphics::AddDefaultScale(sf::Vector2f vector) const
{
...
sf::Vector2f Graphics::GetMousePos()
{
	sf::Vector2f mousePos = (sf::Vector2f)sf::Mouse::getPosition(GetRenderWindow());
	mousePos = AddDefaultRotation(mousePos);
	mousePos = AddDefaultScale(mousePos);
	mousePos = AddDefaultTranslation(mousePos);
	Draw(mousePos, 3.0f, sf::Color::Red);
	return mousePos;
}
